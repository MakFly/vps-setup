...
Path to the directory containing the Ansible playbooks. VPS Setup looks for the main playbook at `<ansiblePath>/playbooks/site.yml`.

```yaml
ansiblePath: "~/.local/share/vps-setup/ansible"
```

Use an absolute path for predictable provisioning from any working directory:
